...
```bash
./gradlew shadowJar
java -jar build/libs/eventb-checker-1.3-all.jar check /path/to/model.zip
java -jar build/libs/eventb-checker-1.3-all.jar info /path/to/model.zip --types --format json
```
...
steps:
  ...
  - uses: eventb-rossi/eventb-checker@v1.3
    with:
